...
describe("checkProperty", () => {
	...
	it("Should test if the element has the correct color", () => {
		...
		_expect(global.browser.getCssProperty).toHaveBeenCalledWith(
			"#elem1",
			"color"
		);
	...
	it("Should test if the element does not have a width of 1px", () => {
		...
		_expect(expectToNotEqual).toHaveBeenCalledWith(
			"1px",
			'CSS attribute of element "#elem2" should not ' + 'contain "1px"'
		);
	});

	it("Should test if the element has the correct name", () => {
		checkProperty(false, "name", "#elem3", false, "element-name");

		_expect(global.browser.getAttribute).toHaveBeenCalledTimes(1);
		_expect(global.browser.getAttribute).toHaveBeenCalledWith("#elem3", "name");
