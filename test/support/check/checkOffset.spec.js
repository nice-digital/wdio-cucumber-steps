...
describe("checkOffset", () => {
	...
	it("Should test if the element is positioned at the expected location", () => {
		checkOffset("#elem1", false, 100, "x");

		_expect(global.browser.getLocation).toHaveBeenCalledTimes(1);
		_expect(global.browser.getLocation).toHaveBeenCalledWith("#elem1", "x");
	...
	it(
		...
		() => {
			...
			_expect(global.browser.getLocation).toHaveBeenCalledTimes(1);
			_expect(global.browser.getLocation).toHaveBeenCalledWith("#elem2", "y");

			_expect(expectToNotEqual).toHaveBeenCalledTimes(1);
			_expect(expectToNotEqual).toHaveBeenCalledWith(
				200,
				'Element "#elem2" should not be positioned at ' + "200px on the y axis"
			);
		}
	);
});
