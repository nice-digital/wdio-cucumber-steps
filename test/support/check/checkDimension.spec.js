...
describe("checkDimension", () => {
	...
	it("Should test the height of an element against a given value", () => {
		...
		_expect(global.browser.getElementSize).toHaveBeenCalledWith("element1");

		_expect(expectToEqual).toHaveBeenCalledTimes(1);
		_expect(expectToEqual).toHaveBeenCalledWith(
			200,
			'Element "element1" should have a height of ' + "200px, but is 200px"
		);
	});
	...
	it("Should test the width of an element against a given value", () => {
		...
		_expect(global.browser.getElementSize).toHaveBeenCalledTimes(1);
		_expect(global.browser.getElementSize).toHaveBeenCalledWith("element2");

		_expect(expectToEqual).toHaveBeenCalledTimes(1);
		_expect(expectToEqual).toHaveBeenCalledWith(
			100,
			'Element "element2" should have a width of ' + "100px, but is 100px"
		);
	});
	...
	it("Should test the height of an element against a given value", () => {
		...
		_expect(global.browser.getElementSize).toHaveBeenCalledTimes(1);
		_expect(global.browser.getElementSize).toHaveBeenCalledWith("element3");
