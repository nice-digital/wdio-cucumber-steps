...
We use [np](https://www.npmjs.com/package/np) for releasing. Run `npm run release:patch` to release a patch version or `npm run release:minor`(major releases have been removed from this version as we expect all major releases to be handled on the master branch).`This will run tests, bump package.json version and create a git tag for you. Or run`npm run release`for more fine grained control, for example`npm run release -- prepatch --tag=alpha --preview --any-branch`.
