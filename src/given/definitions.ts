...
Given(/^the element "([^"]*)?" is( not)* visible$/, isDisplayed).withDocs(
	"Check the (in)visibility of a element"
);

Given(/^the element "([^"]*)?" is( not)* enabled$/, isEnabled).withDocs(
	"Check if a element is (not) enabled"
);

Given(/^the element "([^"]*)?" is( not)* selected$/, checkSelected).withDocs(
	"Check if a element is (not) selected"
);

Given(/^the checkbox "([^"]*)?" is( not)* checked$/, checkSelected).withDocs(
...
Given(/^the title is( not)* "([^"]*)?"$/, checkTitle).withDocs(
...
Given(/^the page url is( not)* "([^"]*)?"$/, checkUrl).withDocs(
...
Given(/^the cookie "([^"]*)?" does( not)* exist$/, checkCookieExists).withDocs(
...
Given(/^a (alertbox|confirmbox|prompt) is( not)* opened$/, checkModal).withDocs(
	"Check if a modal is opened"
);

Given("I debug", debug).withDocs(
	"Add a breakpoint to stop the running browser and give you time to jump into it and check the state of your application ([WDIO Help on Debug](http://webdriver.io/api/utility/debug.html))."
);

Given(
	/^I am logged in to (beta|live|test) Accounts with username "([A-z0-9_@.]+)" and password "([A-z0-9_]+)"$/,
	accountsLogin
).withDocs(
	"Log into a specific version of Nice accounts independently of using TopHat. Username and Password should be names of environment variables (eg, Given I am logged in to beta Accounts with username 'ACCOUNTS_EMAIL' and password 'ACCOUNTS_PASSWORD'). If this is used remember to redirect back to where you expect to be"
);

Given(/^I am logged out of NICE accounts$/, accountsLogOut).withDocs(
	"Log out of NICE accounts. If this is used remember to redirect back to where you expect to be"
);
