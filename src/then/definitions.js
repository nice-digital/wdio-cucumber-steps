...
export const stepDefinitions = [
	...
	Then(/^I expect that element "([^"]*)?" does( not)* appear exactly "([^"]*)?" times$/, checkIfElementExists)
	...
	Then(/^I expect that element "([^"]*)?" becomes( not)* visible$/, waitForVisible)
	...
	Then(/^I expect that element "([^"]*)?"( not)* contains the same text as element "([^"]*)?"$/, compareText)
		.withDocs("Compare the text of two elements"),

	Then(/^I expect that (button|element) "([^"]*)?"( not)* matches the text "([^"]*)?"$/, checkEqualsText)
		.withDocs("Check if a element or input field equals the given text"),

	Then(/^I expect that (button|element) "([^"]*)?"( not)* contains the text "([^"]*)?"$/, checkContainsText)
	...
	Then(/^I expect that the url is( not)* "([^"]*)?"$/, checkURL)
		.withDocs("Check if the the URL of the current browser window/tab is (not) a certain string"),

	Then(/^I expect that the path is( not)* "([^"]*)?"$/, checkURLPath)
		.withDocs("Check if the path of the URL of the current browser window/tab is (not) a certain string"),

	Then(/^I expect the url to( not)* contain "([^"]*)?"$/, checkInURLPath)
		.withDocs("Check if the URL of the current browser window/tab (doesn't) contain(s) a certain string"),

	Then(/^I expect that the( css)* attribute "([^"]*)?" from element "([^"]*)?" is( not)* "([^"]*)?"$/, checkProperty)
	...
	Then(/^I expect that element "([^"]*)?" is( not)* positioned at ([\d]+)px on the (x|y) axis$/, checkOffset)
	...
	Then(/^I wait on element "([^"]*)?"(?: for (\d+)ms)*(?: to( not)* (be checked|be enabled|be selected|be visible|contain a text|contain a value|exist))*$/, waitFor)
		.withOptions({
			wrapperOptions: {
				retry: 3
			}
		})
		.withDocs("Wait for a element to be checked, enabled, selected, visible, contain a certain value or text or to exist"),
